<?php

return array (
  'item_deleted' => 'مورد حذف شد !',
  'blog_slider' => 'اسلایدر',
  'category' => 'دسته بندی',
  'user_created' => 'حساب کاربری با موفقیت ایجاد شد',
  'password_changed' => 'کلمه عبور شما با موفقیت تغییر داده شد.',
  'current_password_invalid' => 'کلمه عبور فعلی صحیح نمیباشد.',
  'product_added_successfully' => 'محصول با موفقیت به سبد خرید افزوده شد!',
  'period_created' => 'دوره پرداخت ثبت گردید.',
  'period_res' => 'دوره پرداخت شد.',
  'period_not_found' => 'پرداخت دوره ای یافت نشد!',
  'transaction_created' => 'تراکنش ایجاد شد.',
  'item_created' => 'مورد با موفقیت ایجاد شد.',
  'post' => 'مطلب',
  'item_updated' => 'مورد با موفقیت ویرایش شد.',
  'building_type' => 'تیپ ساختمان',
  'item' => 'ایتم',
  'host' => 'اقامتگاه',
  'image' => 'تصویر',
  'video' => 'ویدئو',
  'file' => 'فایل',
  'gateway' => 'درگاه',
  'transportation' => 'حمل و نقل',
  'discount_code' => 'کد تخفیف',
  'discount_code_delete' => 'کد تخفیف حذف گردید.',
  'updated' => 'بروزرسانی شد.',
  'all_teams' => 'همه تیم ها',
  'team_not_found' => 'تیم یافت نشد',
  'canceled' => 'لغو شده',
  'registering' => 'در حال ثبت نام',
  'ready' => 'آماده اعزام',
  'arrived' => 'در مقصد',
  'exited' => 'در مسیر بازگشت',
  'archived' => 'انجام شده',
  'large' => 'بزرگ',
  'medium' => 'متوسط',
  'thumbnail' => 'کوچک',
  'login_form_title' => 'ورود به سایت',
  'remember_me' => 'مرا به یاد داشته باش',
  'forgot_password' => 'فراموشی کلمه عبور',
  'login' => 'ورود',
  'email' => 'ایمیل',
  'categories' => 'دسته بندی ها',
  'add_new' => 'ثبت مورد جدید',
  'edit_category' => 'ویرایش دسته بندی',
  'save' => 'ذخیره',
  'category_name' => 'نام دسته بندی',
  'slug' => 'اسلاگ',
  'description' => 'شرح',
  'optional' => 'اختیاری',
  'add_category' => 'افزودن',
  'add_item' => 'افزودن',
  'delete_item' => 'حذف مورد',
  'delete_item_text' => 'آیا از حذف مورد اطمینان دارید؟',
  'delete' => 'حذف',
  'madadjoo' => 'مددجو',
  'cancel' => 'انصراف',
  'add_new_category' => 'دسته بندی جدید اضافه کنید.',
  'add_image' => 'افزودن تصویر',
  'comment_list' => 'لیست نظرات',
  'image_upload' => 'بارگزاری تصویر',
  'title' => 'عنوان',
  'upload' => 'بارگزاری',
  'file_uploaded' => 'فایل با موفقیت بارگزاری شد',
  'image_title' => 'عنوان تصویر',
  'select_image' => 'انتخاب عکس',
  'change_size_to' => 'تغییر اندازه به',
  'full_size' => 'اندازه اصلی',
  'images_id' => 'شناسه تصویر',
  'untitled_photo' => 'بدون عنوان',
  'uploaded_on' => 'بارگزاری شده در',
  'zoom_in' => 'بزرگ نمایی',
  'address' => 'آدرس',
  'image_url' => 'آدرس تصویر',
  'image_tag' => 'تگ تصویر',
  'post_list' => 'لیست مطالب',
  'subtitle' => 'زیر عنوان',
  'author' => 'نویسنده',
  'posted_at' => 'تاریخ انتشار',
  'Categories' => 'دسته بندی ها',
  'new_post' => 'مطلب جدید',
  'display_title' => 'عنوانی که نمایش داده میشود',
  'display_subtitle' => 'توضیحات بیشتری که در مورد پست نمایش داده میشود (اختیاری)',
  'post_slug' => 'اسلاگ',
  'i_e' => 'مثال:',
  'is_published' => 'وضعیت انتشار',
  'published' => 'منتشر شده',
  'not_published' => 'پیش نویس',
  'posted_at_help' => 'میخواهید این پست در چه زمانی منتشر شود؟ اگر میخواهید در زمانی بعد از زمان فعلی ( :now) منتشر شود باید با فرمت :format یک زمان در آینده را وارد کنید.',
  'post_text' => 'متن مطلب',
  'custom_view_file' => 'نمایش در صفحه اختصاصی',
  'custom_view_file_help' => 'چیزی که در این قسمت نوشته میشود باید نام صفحه اختصاصی طراحی شده باشد. در صوتی که صفحه ای وجود ندارد چیزی وارد نکنید.',
  'tags' => 'تگ ها',
  'tags_help' => 'اگر چیزی وارد نشود به صورت اتوماتیک ساخته میشوند',
  'meta_desc' => 'توضیحات متا',
  'short_desc' => 'توضیحات کوتاه',
  'short_desc_help' => 'توضیحاتی که میخواهید در صفحه اصلی نمایش داده شود',
  'featured_images' => 'تصویر عنوان',
  'auto_size_image_help' => 'به صورت پیش فرض بر اساس اولین تصویر آپلود شده سایز های دیگر ساخته میشوند، در صورتی که میخواهید سایز خاصی را هم آپلود کنید کلیک کنید:',
  'more_size' => 'سایز های بیشتر',
  'no_categories' => 'بدون دسته بندی',
  'hor_position' => 'موقعیت افقی',
  'text' => 'متن',
  'one' => 'یک',
  'left' => 'چپ',
  'right' => 'راست',
  'center' => 'وسط',
  'two' => 'دو',
  'tree' => 'سه',
  'btn_text' => 'متن دکمه',
  'btn_link' => 'لینک دکمه',
  'btn_dir' => 'جهت نمایش دکمه',
  'account' => 'پیشخوان نیکوکار',
  'welcome' => 'خوش آمدید.',
  'name_family' => 'نام و نام خانوادگی',
  'username' => 'نام کاربری',
  'national_code' => 'کد ملی',
  'sex' => 'جنسیت',
  'phone' => 'شماره تلفن',
  'mobile' => 'شماره تلفن',
  'change_password' => 'تغییر کلمه عبور',
  'edit_information' => 'ویرایش اطلاعات',
  'payment_report' => 'گزارش پرداخت ها',
  'buy_report' => 'گزارش سفارشات',
  'gallery' => 'نگارخانه',
  'click_here' => 'کلیک کنید',
  'image_gallery_ashraf' => 'نگارخانه خیریه اشرف الانبیاء (ص)',
  'articles_preview' => 'نیم نگاهی به مقالات',
  'now_password' => 'کلمه عبور فعلی',
  'new_password' => 'کلمه عبور جدید',
  'repeat_new_password' => 'تکرار کلمه عبور جدید',
  'random_photos' => 'تصاویر تصادفی',
  'photos_gallery' => 'گالری تصاویر',
  'view' => 'مشاهده',
  'email_or_mobile' => 'ایمیل یا موبایل یا نام کاربری',
  'enter_email_mobile' => 'ایمیل یا شماره موبایل',
  'password' => 'کلمه عبور',
  'enter_password' => 'کلمه عبور',
  'please_waite' => 'منتظر بمانید...',
  'login_page_title' => 'ورود به سایت',
  'register_form_title' => 'فرم ثبت نام',
  'repeat_password' => 'تکرار کلمه عبور',
  'repeat_repeat_password' => 'تکرار کلمه عبور',
  'register_btn' => 'ثبت نام',
  'duplicate_email' => 'این نام کاربری در سیستم وجود دارد.',
  'register_page_title' => 'حساب کاربری ندارید؟ ایجاد کنید.',
  'register_page_description' => 'برای استفاده کامل از امکانات سایت حساب کاربری ایجاد کنید.',
  'register' => 'عضویت',
  'search_blog' => 'جستجو مطالب',
  'search_results_for' => 'نتایج جستجو',
  'search_results_null' => 'متاسفانه جستجو نتیجه ای در بر نداشت.',
  'search_results_cant_display' => 'نتایج جستجو قابل نمایش نیست.',
  'latest_post' => 'آخرین مطالب',
  'read_more' => 'ادامه مطلب',
  'latest_posts' => 'آخرین مطالب',
  'watch_more_videos' => 'نمایش ویدئو های بیشتر',
  'comment' => 'دیدگاه',
  'posted' => 'ارسال شده',
  'comments' => 'نظرات',
  'no_comment_yet' => 'هنوز نظری ثبت نشده است. ',
  'leave_a_comment' => 'نظر دهید',
  'name' => 'نام',
  'comment_body' => 'متن نظر',
  'submit' => 'ثبت',
  'payment_list' => 'در انتظار پرداخت',
  'periodic_payment' => 'کمک ماهانه',
  'paid' => 'پرداخت شده',
  'amount' => 'مبلغ',
  'start_date' => 'تاریخ شروع',
  'status' => 'وضعیت',
  'pay' => 'پرداخت',
  'period' => 'دوره',
  'change_status' => 'تغییر وضعیت',
  'are_you_sure' => 'آیا مطمئن هستید؟',
  'yes_i_sure' => 'بله مطمئنم!',
  'tableau_and_wreath' => 'تابلو و تاج گل',
  'product_title' => 'عنوان محصول',
  'count' => 'تعداد',
  'fee' => 'قیمت واحد',
  'off' => 'تخفیف',
  'sub_price' => 'جمع کل',
  'toman' => 'تومان',
  'pay_amount' => 'مبلغ قابل پرداخت',
  'continue_shopping' => 'ادامه ثبت سفارش',
  'view_basket' => 'مشاهده سبد خرید',
  'add_to_cart' => 'افزودن به سبد',
  'size' => 'اندازه',
  'properties' => 'ویژگی ها',
  'product_description' => 'توضیحات محصول',
  'login/register' => 'ورود/ثبت نام',
  'send_information' => 'اطلاعات ارسال',
  'payment_information' => 'اطلاعات پرداخت',
  'customer_information' => 'اطلاعات مشتری',
  'zip_code' => 'کد پستی',
  'change_address' => 'افزودن آدرس',
  'how_to_send' => 'نحوه ارسال',
  'free' => 'رایگان',
  'send_time' => 'زمان ارسال',
  'max_send_time_your_order' => 'حداکثر زمان ارسال سفارش شما',
  'work_day' => 'روز کاری',
  'order_title' => 'عنوان سفارش',
  'view_detail' => 'نمایش جزییات',
  'for' => 'بابت',
  'payment_gateway' => 'درگاه پرداخت',
  'cooperation' => 'توضیحات ما',
  'please_select' => 'لطفاْ انتخاب نمایید.',
  'daily' => 'روزانه',
  'monthly' => 'ماهانه',
  'every_3_months' => 'هر سه ماه یک بار',
  'every_6_months' => 'هر شش ماه یک بار',
  'yearly' => 'سالانه',
  'icon' => 'آیکون',
  'value' => 'مقدار',
  'display_statistics' => 'آمار نمایشی',
  'copyright' => 'کلیه حقوق این وب سایت متعلق به موسسه خیریه اشرف الانبیاء (ص) می باشد',
  'html_title' => 'موسسه خیریه اشرف الانبیاء(ص)',
  'Loading' => 
  array (
    '' => 
    array (
      '' => 
      array (
        '' => 'منتظر بمانید...',
      ),
    ),
  ),
  'Disable_Preloader' => 'غیر فعال سازی پیش نمایش',
  'logout' => 'خروج',
  'buy_basket' => 'سبد خرید',
  'home' => 'خانه',
  'vows' => 'نذورات',
  'financial_aids' => 'کمک های نقدی',
  'Periodic_assistance' => 'کمک ماهانه',
  'FAQ' => 'سوالات متداول',
  'help_desk' => 'کمک یار',
  'support' => 'پیشتیبانی',
  'dashboard' => 'میزکار',
  'blog' => 'مطالب',
  'post_add' => 'افزودن مطلب',
  'blog_posts' => 'مطالب',
  'blog_comments' => 'نظرات',
  'all_blog_comments' => 'لیست نظرات',
  'pending_blog_comments' => 'در انتظار تایید',
  'blog_categories' => 'دسته بندی مطالب',
  'all_blog_categories' => 'لیست دسته بندی ها',
  'blog_images' => 'تصاویر',
  'all_blog_images' => 'همه تصاویر',
  'add_blog_images' => 'افزودن تصویر',
  'blog_setting' => 'تنظیمات سایت',
  'more_setting' => 'تنظیمات بیشتر',
  'site_info' => 'اطلاعات سایت',
  'link' => 'لینک',
  'adv_links' => 'لینک های تبلیغی',
  'adv_card' => 'کارت تبلیغاتی',
  'adv_bar' => 'نوار متحرک',
  'social_media' => 'شبکه های اجتماعی',
  'store' => 'فروشگاه',
  'product_add' => 'افزودن محصول',
  'product_list' => 'لیست محصولات',
  'store_category' => 'دسته بندی فروشگاه',
  'store_items' => 'آیتم های فروشگاه',
  'manage_orders' => 'مدیریت سفارشات',
  'store_setting' => 'تنظیمات فروشگاه',
  'photos' => 'تصاویر',
  'videos' => 'ویدئو ها',
  'caravans' => 'کاروان ها',
  'caravans_list' => 'لیست کاروان ها',
  'hosts_list' => 'لیست مهمانسرا ها',
  'building_projects' => 'سایه رضوان',
  'building_dashboard' => 'میز کار',
  'building_types' => 'تیپ ساختمان ها',
  'building_archive' => 'آرشیو پروژه ها',
  'follow_project' => 'دنبال کردن پروژه',
  'unfollow_project' => 'دنبال نکردن',
  'Charity' => 'خیریه',
  'request_hook' => 'درخواست صندوق',
  'charity_setting' => 'تنظیمات خیریه',
  'payment_titles' => 'عناوین پرداخت',
  'hooks_types' => 'انواع صندوق ها',
  'users_management' => 'مدیریت حساب های کاربری',
  'users_list' => 'لیست کاربران',
  'permissions_list' => 'لیست دسترسی ها',
  'roles_list' => 'لیست نقش ها',
  'teams_list' => 'لیست تیم ها',
  'setting' => 'تنظیمات',
  'cities' => 'شهرها',
  'gateway_pay' => 'درگاه پرداخت',
  'enter_category_title' => 'عنوان دسته بندی را وارد نمایید.',
  'add' => 'افزودن',
  'edit' => 'ویرایش',
  'categories_list' => 'لیست دسته بندی ها',
  'enter_title' => 'عنوان را وارد نمایید...',
  'enter_text' => 'تایپ نمایید',
  'publish_time' => 'زمان انتشار',
  'publish_status' => 'وضعیت انتشار',
  'draft' => 'پیش نویس',
  'publish' => 'انتشار',
  'items' => 'ایتم ها',
  'in_progress' => 'در حال بررسی',
  'closed' => 'بسته شده',
  'filter' => 'فیلتر',
  'normal_ticket' => 'تیکت عادی',
  'by' => 'توسط',
  'set_project_finish' => 'اعلام پایان پروژه',
  'project_info' => 'اطلاعات پروژه',
  'predict_end_date' => 'تخمین تاریخ پایان',
  'actual_end_date' => 'تاریخ قطعی پایان پروژه',
  'show_in_map' => 'نمایش روی نقشه',
  'edit_item' => 'ویرایش',
  'related_persons' => 'افراد مرتبط',
  'project_manager' => 'مدیر پروژه',
  'project_officer' => 'کارشناس پروژه',
  'project_worker' => 'مجری پروژه',
  'project_watcher' => 'دنبال کننده',
  'progress_percent' => 'درصد پیشرفت',
  'certain' => 'قطعی',
  'not_verified' => 'تایید نشده',
  'add_new_building' => 'تعریف پروژه جدید',
  'project_type_title' => 'عنوان تیپ پروژه',
  'project_title' => 'عنوان پروژه',
  'end_date' => 'تاریخ پایان',
  'city' => 'شهر',
  'lat' => 'عرض جغرافیایی',
  'long' => 'طول جغرافیایی',
  'coordination' => 'مختصات مکانی',
  'save_and_continue' => 'ذخیره و ادامه',
  'percent' => 'درصد',
  'confirm_re_open_ticket' => 'شما در حال بازگشایی یک تیکت آرشیو شده میباشد، در صورتی که میخواهید تیکت بازگشایی شود تایید کنید',
  'ticket_set_predict_percent' => 'ثبت کننده درصد پیشرفت را برای این مرحله اعلام کرده است.',
  'actual_percent' => 'درصد پیشرفت تایید شده',
  'approve' => 'تایید',
  'archive_ticket' => 'ثبت و آرشیو تیکت',
  'action' => 'اعمال',
  'weight' => 'وزن',
  'priority' => 'اولویت',
  'ticket' => 'تیکت',
  'critical' => 'بحرانی',
  'high_priority' => 'اولویت بالا',
  'normal_priority' => 'عادی',
  'low_priority' => 'اولویت پایین',
  'type' => 'نوع',
  'progress_ticket' => 'پیشرفت کار',
  'users' => 'کاربران',
  'attachment' => 'فایل ضمیمه',
  'project_page' => 'صفحه پروژه',
  'options' => 'گزینه ها',
  'accept_ticket' => 'قبول بررسی',
  'close_ticket' => 'بستن تیکت',
  're_open_ticket' => 'بازگشایی تیکت',
  're_open_project' => 'بازگشایی پروژه',
  'Refer_to' => 'ارجاع به شخص دیگر',
  'ticket_info' => 'اطلاعات تیکت',
  'creator' => 'ایجاد کننده',
  'create_date' => 'تاریخ ایجاد',
  'ticket_id' => 'شماره تیکت',
  'history' => 'تاریخچه',
  'ticket_creation' => 'ایجاد تیکت',
  'add_note' => 'افزودن توضیحات',
  'refer' => 'ارجاع',
  'assign_to_self' => 'پذیرش بررسی',
  'close' => 'بستن',
  'approved' => 'تایید شد',
  'reject' => 'عدم تایید',
  'caravan' => 'کاروان',
  'person_count' => 'تعداد افراد',
  'date' => 'تاریخ',
  'depart' => 'حرکت',
  'duty' => 'مسئول',
  'add_caravan' => 'ثبت کاروان جدید',
  'active_caravan' => 'کاروان های در حال انجام',
  'capacity' => 'ظرفیت',
  'no_limit' => 'بدون محدودیت',
  'province' => 'استان',
  'sub_province' => 'شهرستان',
  'departure' => 'مبداء',
  'budget' => 'بودجه',
  'executer' => 'مجری',
  'transport_type' => 'نوع وسیله نقلیه',
  'entrance' => 'ورود',
  'exit' => 'خروج',
  'get_back' => 'برگشت',
  'empty' => 'خالی',
  'gender' => 'جنسیت',
  'male' => 'آقا',
  'female' => 'خانم',
  'both' => 'هردو',
  'close_caravan_registration_warn' => 'در صورتی که ثبت نام افراد در لیست این کاروان به اتمام رسیده است، برای ادامه مراحل اعزام تایید کنید.',
  'arrive_caravan_warn' => 'در صورتی که کاروان مورد نظر به محل اقامت خود رسیده است تایید کنید.',
  'exit_caravan_warn' => 'برای ثبت خروج کاروان از محل اقامت تایید کنید.',
  'archive_caravan_warn' => 'در صورتی که این کاروان به اتمام رسیده است برای آرشیو اطلاعات تایید کنید.',
  'cancel_caravan_warn' => 'در صورتی که این کاروان لغو شده و سفر آن انجام نشده است تایید کنید.',
  'cancel_caravan' => 'لغو کاروان',
  'back_caravan_status_warn' => 'در صورتی که میخواهید وضعیت اعزام کاروان به مرحله قبل بازگردد تایید کنید.',
  'back_caravan_status' => 'بازگشت وضعیت اعزام کاروان',
  'sh_code' => 'شماره شناسنامه',
  'mr' => 'آقا',
  'mrs' => 'خانم',
  'family' => 'نام خانوادگی',
  'father_name' => 'نام پدر',
  'birth_date' => 'تاریخ تولد',
  'check' => 'بررسی',
  'age' => 'سن',
  'use_count' => 'تعداد دفعات استفاده',
  'accept' => 'موافقت',
  'caravan_status' => 'وضعیت کاروان',
  'accepted' => 'تایید شده',
  'rejected' => 'عدم تایید',
  'pending' => 'در حال بررسی',
  'new_register' => 'ثبت نام جدید',
  'next_step' => 'مرحله بعد',
  'previous_step' => 'مرحله قبل',
  'destination' => 'مقصد',
  'caravan_info' => 'مشخصات کاروان',
  'Workflow' => 'گردش کار',
  'min' => 'حداقل',
  'max' => 'حداکثر',
  'custom_fields' => 'فیلد های اضافه',
  'input' => 'ورودی متن',
  'textarea' => 'ورودی متن زیاد',
  'number' => 'ورودی عددی',
  'date_input' => 'ورودی تاریخ',
  'time_input' => 'ورودی ساعت',
  'required' => 'اجباری',
  'recover_title_warn' => 'برای بازیابی آیتم حذف شده تایید کنید.',
  'recover' => 'بازیابی',
  'custom_payment_types' => 'انواع دیگر کمک نقدی',
  'display_items' => 'آیتم هایی که نمایش داده میشوند',
  'deleted_items' => 'آیتم های حذف شده',
  'recover_item' => 'بازیابی مورد',
  'description_more' => 'توضیحات بیشتر',
  'iframe_help' => 'لینک آی فریم ساخته شده توسط میزبان ویدئو رو در این قسمت قرار دهید بطوری که با تگ آی فریم شروع و تمام شود',
  'add_gallery_category' => 'افزودن دسته بندی نگارخانه',
  'back' => 'برگشت',
  'bank_information' => 'اطلاعات بانک',
  'bank_name' => 'نام بانک',
  'id' => 'شناسه',
  'account_number' => 'شماره حساب',
  'sheba_number' => 'شماره شبا',
  'branch' => 'شعبه',
  'card_number' => 'شماره کارت',
  'active' => 'فعال',
  'inactive' => 'غیر فعال',
  'gateway_pay_info' => 'اطلاعات درگاه پرداخت',
  'merchant_id' => 'مرچنت آی دی',
  'public_key' => 'کلید عمومی',
  'private_key' => 'کلید خصوصی',
  'terminal_id' => 'کلید ترمینال',
  'online' => 'آنلاین',
  'cart_to_cart' => 'کارت به کارت',
  'send_to_account' => 'واریز به حساب',
  'add_transport' => 'افزودن حمل و نقل',
  'day' => 'روز',
  'cost' => 'هزینه',
  'expire_date' => 'تاریخ انقضاء',
  'discount_persent' => 'درصد تخفیف',
  'discount_maximum' => 'حداکثر تخفیف',
  'usage_count_per_user' => 'تعداد استفاده هر کاربر',
  'duplicate_discount_code' => 'کد تخفیف در سیستم موجود میباشد.',
  'add_discount_code' => 'افزودن کد تخفیف',
  'prefix' => 'پیشوند',
  'suffix' => 'پسوند',
  'inventories' => 'موجودی',
  'price' => 'مبلغ',
  'without_color' => 'بدون رنگ',
  'by_color' => 'با رنگ',
  'add_color' => 'افزودن رنگ',
  'pay_gateway' => 'درگاه های پرداخت',
  'pay_on_place' => 'پرداخت در محل',
  'ready_for_send' => 'مدت زمان ارسال',
  'select' => 'انتخاب',
  'product_category' => 'دسته بندی محصولات',
  'add_product_category' => 'افزودن دسته بندی محصولات',
  'teams' => 'تیم ها',
  'roles' => 'نقش ها',
  'key' => 'کلید',
  'users_has_this_permission' => 'کاربرانی که این دسترسی را دارند',
  'roles_has_this_permission' => 'نقش هایی که این دسترسی را دارند',
  'team' => 'تیم',
  'role' => 'نقش',
  'add_new_user' => 'افزودن کاربر جدید',
  'register_date' => 'تاریخ ثبت نام',
  'roles_user_has' => 'نقش هایی که این کاربر دارد',
  'permission_user_has' => 'دسترسی هایی که این کاربر دارد',
  're-password' => 'ورود مجدد رمز عبور',
  'no_found_any' => 'هیچ آیتمی یافت نشد.',
  'user_that_has_the_role' => 'کاربرانی که این نقش را دارند',
  'permission_that_role_has' => 'دسترسی هایی که این نقش دارد',
  'test' => 'امتحانی.',
  'that' => 'که',
  'user' => 'کاربر',
  'permission' => 'دسترسی',
  'permissions' => 'دسترسی ها',
  'roles_permission' => 'نقش ها و دسترسی ها',
  'groups' => 'مدیریت گروه ها',
  'final_register' => 'ثبت نهایی',
  'donate' => 'مشارکت',
  'additional_info' => 'اطلاعات تکمیلی',
  'doc_upload' => 'آپلود مدارک',
  'new_doc_upload' => 'بارگزاری مدرک جدید',
  'uploaded_docs' => 'مدارک آپلود شده',
  'final_submit' => 'ثبت نهایی',
  'last_login' => 'آخرین لاگین',
  'msd_house' => 'خانه فرهنگی مشهد مقدس',
  'pending_list' => 'لیست در انتظار',
  'start' => 'شروع',
  'form_password' => 'کلمه عبور',
  'features' => 'ویژگی ها',
  'my_profile' => 'پروفایل من',
  'search' => 'جستجو',
  'add_blog_category' => 'افزودن دسته بندی',
  'commentss' => 'دیدگاهی',
  'post_count' => 'تعداد پست',
  'persons_list' => 'لیست افراد',
  'no_item_find' => 'هیچ آیتمی پیدا نشد ! ',
  'charity_dashboard' => 'صفحه مدیریت خیریه',
  'payment_periods' => 'دوره های پرداخت',
  'payment_period' => 'دوره پرداخت',
  'period_interval' => 'دوره تکرار',
  'in_days' => '(روز)',
  'payment_title' => 'عنوان پرداخت',
  'payment_pattern' => 'الگو پرداخت',
  'payment_selection' => 'سرفصل پرداخت',
  'the_victim' => 'قربانی',
  'excuse' => 'عقیقه',
  'non-cash' => 'غیر نقدی',
  'cash' => 'نقدی',
  'user_reviews' => 'نظرات کاربران',
  'add_size' => 'افزودن سایز',
  'yes_delete_id' => 'بله! حذف شود.',
  'period_inactived' => 'پرداخت دوره ای غیر فعال شد.',
  'activate' => 'فعال سازی',
  'inactivate' => 'غیر فعال سازی',
  'unpaid' => 'پرداخت نشده',
  'sms_box' => 'صندوق دریافت پیامک',
  'no_any_post' => 'هیچ پستی یافت نشد.',
  'not_found_any_data' => 'اطلاعاتی یافت نشد',
  'please_insert_post' => 'لطفاً پست جدید اضافه نمایید.',
  'messages' => 
  array (
    'image_update_disabled' => 'بارگزاری تصویر غیر فعال می باشد.',
  ),
  'please_no_any_comment' => 'هیچ نظری در سیستم یافت نشد.',
  'blog_album' => 'البوم مطالب',
  'show' => 'نمایش',
  'show_site' => 'نمایش سایت',
  'approve_image_text' => 'آیا از ثبت تصویر اصلی دسته بندی اطمینان دارید؟',
  'period_list' => 'لیست دوره ها',
  'it\'s_over_date' => 'زمان سر رسید',
  'payment_date' => 'تاریخ پرداخت',
  'period_payment_list' => 'لیست نیکوکاران کمک ماهانه',
  'next_payment_date' => 'تاریخ پرداخت بعدی',
  'send_sms' => 'ارسال پیامک',
  'for_submit_period_payment_first_login_or_register' => 'برای ثبت پرداخت دوره ای ابتدا وارد سیستم شوید یا اگر عضو سامانه نیستید عضویت خود را تکمیل نمایید.',
  'waiting' => 'منتظر بررسی',
  'admin_review' => 'بررسی مدیر',
  'payment_status' => 'وضعیت پرداخت',
  'waiting_list' => 'لیست منتظر بررسی',
  'payment_approved' => 'تاییده پرداخت انجام شد.',
  'payment_approve' => 'تاییده پرداخت',
  'approved_list' => 'لیست تایید شده',
  'approve_time' => 'زمان تایید',
  'approve_user' => 'کاربر تایید کننده',
  'other_payments' => 'سایر پرداخت ها',
  'patern' => 'الگو',
  'language' => 'زبان',
  'model' => 'مدل',
  'code' => 'کد',
  'new_orders' => 'سفارشات جدید',
  'approved_orders' => 'سفارشات تایید شده',
  'ready_for_sent' => 'آماده ارسال',
  'sent' => 'ارسال شده',
  'final_orders' => 'نهایی',
  'for_submit_order_need_to_login' => 'برای ثبت سفارش نیاز است وارد سیستم شوید.',
  'receiver_name' => 'نام دریافت کننده',
  'add_new_address' => 'تکمیل اطلاعات',
  'no_any_address_submit' => 'هیچ آدرسی در سیستم ثبت نشد.',
  'address_added' => 'آدرس اضافه گردید.',
  'address_deleted' => 'آدرس حذف گردید.',
  'your_location' => 'موقعیت شما',
  'payment_type' => 'نحوه پرداخت',
  'target_amount' => 'مبلغ هدف',
  'payment_code' => 'شناسه پرداخت',
  'add_champion' => 'افزودن کمپین',
  'please_click_or_drop_and_down_file_for_upload' => 'برای بارگذاری فایل لطفاً کلیک نمایید یا فایل به در اینجا رها نمایید.',
  'show_on' => 'نمایش در',
  'show_on_last_post' => 'نمایش در قسمت اخرین مطالب',
  'show_on_articles' => 'نمایش در قسمت مقالات',
  'preview' => 'پیش نمایش',
  'edit_post' => 'ویرایش مطلب',
  'show_post' => 'نمایش مطلب',
  'date_not_valid' => 'تاریخ وارد شده صحیح نمی باشد.',
  'author_website' => 'آدرس سایت',
  'first' => 'اول',
  'last' => 'آخر',
  'account_information' => 'اطلاعات حساب کاربری',
  'login_to_ashraf_crm' => 'ورود به CRM موسسه خیریه اشرف الانبیاء (ص)',
  'transaction_file' => 'فایل تراکنش',
  'download_sample_file' => 'دانلود فایل نمونه',
  'register_by_excel' => 'ثبت دسته ای اسامی با فایل اکسل',
  'rial' => 'ریال',
  'amount_rial' => 'مبلغ به ریال',
  'is_going_connect_to_gateway' => 'در حال انتقال به درگاه پرداخت...',
  'ashraf' => 'موسسه خیریه اشرف الانبیاء (ص)',
  'continue_post' => 'ادامه مطلب...',
  'your_comment_send_successfully' => 'دیدگاه شما با موفقیت ارسال شد.',
  'your_comment_send_successfully_and_show_after_approve' => 'دیدگاه شما با موفقیت ارسال شد و پس از تایید نمایش داده میشود.',
  'description_meta' => 'توضیحات متا',
  'description_small' => 'توضیحات کوتاه',
  'champion' => 'کمپین',
  'receive' => 'دریافت شده',
  'count_people' => 'تعداد افراد شرکت کننده',
  'notification_template' => 'الگو پیام ها',
  'donate_pay' => 'مبلغ کمک',
  'projects' => 'پروژه های',
  'champions' => 'کمپین ها',
  'projects_gallery' => 'نگارخانه پروژه ها',
  'patron_finance_projects' => 'حامی مالی پروژه ها شوید',
  'patron' => 'حامی',
  'transaction_success' => 'پرداخت شما با موفقیت انجام شد.',
  'tracking_code' => 'شماره پیگیری پرداخت',
  'karim' => 'هوالکریم',
  'support_forms_list' => 'لیست فرم‎‌های حمایتی',
  'charity_support_title' => 'عناوین فرم‌های حمایت',
  'form' => 'فرم',
  'menu' => 'منو',
  'sum_price' => 'جمع مبلغ',
  'transaction_count' => 'نعداد تراکنش',
  'from' => 'از',
  'to' => 'تا',
  'reports' => 'گزارشات پرداخت',
  'charity_period' => 'کمک ماهانه',
  'charity_vow' => 'نذورات',
  'charity_donate' => 'خیریه',
  'payment_chart' => 'نمودار پرداخت ها',
  'active_users' => 'کاربران فعال',
  'image_update_disabled' => 'بروزرسانی تصویر غیرفعال شد',
  'translation_maganger' => 'مترجم سایت',
  'password_recovery' => 'بازیابی کلمه عبور',
  'password_recovery_instructions' => 'راهنمایی بازیابی رمز عبور',
  'send_password_reset_link' => 'ارسال لینک بازنشانی کلمه عبور',
  'permissions_team_list_title' => 'عنوان لیست مجوزها',
  'permission_team_list_user' => 'کاربران',
  'permission_team_list_roles' => 'نقش ها',
  'tages' => 'تگ ها',
  'information' => 'اطلاعات',
  'select_color' => 'انتخاب رنگ',
  'it\\\'s_over_date' => 'تمام شده است',
  'show_permissions' => 'نمایش دسترسی ها',
  'error' => 'خطا',
  'done' => 'انجام شد',
  'item_edited' => 'مورد ویرایش شد',
  'item_added' => 'مورد اضافه شد',
  'category_delete' => 'حذف دسته بندی',
  'added' => 'اضافه شد',
  'product' => 'محصول',
  'deleted' => 'حذف شد',
  'national_code_error' => 'خطای کد ملی !',
  'back_to_main_page' => 'بازگشت به صفحه اصلی',
  'nothing_to_display' => 'مطلبی برای نمایش وجود ندارد.',
  'Specific_page' => 'صفحه خاص',
  'page' => 'صفحه',
  'page_name' => 'نام صفحه',
  'page_category_select' => 'دقت نمایید یا میتوانید مطلب را به صورت پست وبلاگ در دسته بندی فوق قرار دهید. یا اگر مطلب مربوط به صفحه خاصی از سایت است یکی از صفحهات زیر را انتخاب نمایید.',
  'add_Specific_page' => 'صفحه خاص ایجاد کنید.',
  'Introducing_us' => 'معرفی ما',
  'faq' => 'سوالات متداول',
  'contact_to_we' => 'ارتباط با ما',
  'specific_page_list' => 'لیست صفحه های خاص',
  'count_news' => 'ادامه',
  'please_click_for_change_profile_picture' => 'برای تغییر تصویر پروفایل کلید نمایید.',
  'enter_name' => 'نام را وارد نمایید.',
  'enter_family' => 'نام خانوادگی را وارد نمایید',
  'enter_national_code' => 'کد ملی را وارد نمایید',
  'enter_email' => 'ایمیل را وارد نمایید',
  'enter_username' => 'نام کاربری را وارد نمایید',
  'reset' => 'از نو',
  'subject' => 'موضوع',
  'enter_subject' => 'موضوع را وارد نمایید',
  'enter_Phone' => 'تلفن همراه را وارد نمایید',
  'message' => 'پیام',
  'enter_message' => 'متن پیام را وارد نمایید',
  'enter_captcha_code' => 'کد تایید را وارد نمایید',
  'send_message' => 'ارسال پیام',
  'completion_profile' => 'تکمیل پروفایل',
  'response' => 'پاسخ',
  'unknown' => 'نامشخص',
  'ref_id' => 'شماره مرجع',
  'ip' => 'ای پی',
  'your_message_successfully_send' => 'پیام شما با موفقیت ارسال شد.',
  'your_message_fail_to_send' => 'ارسال پیام ناموفق بود.',
  'captcha_code_fail' => 'کد تایید اشتباه میباشد.',
  'question' => 'سوال',
  'fail' => 'ناموفق',
  'success' => 'موفق',
  'enter_mobile' => 'شماره موبایل را وارد نمایید',
  'SUCCEED' => 'موفق',
  'FAILED' => 'ناموفق',
  'new' => 'جدید',
  'read' => 'خوانده شده',
  'list' => 'لیست',
  'verify_phone' => 'فعال سازی موبایل',
  'enter_sms_send_code' => 'لطفاً کد ارسال شده را وارد نمایید.',
  'timeout' => 'اتمام زمان',
  'try_again' => 'مجدد سعی نمایید.',
  'verify_email' => 'فعال سازی ایمیل',
  'enter_email_send_code' => 'لطفاً کد ارسال شده را وارد نمایید.',
  'login_successfully' => 'ورود موفقیت آمیز بود.',
  'addresses' => 'آدرس ها',
  'not_valid' => 'تایید نشده',
  'send_verify_sms' => 'ارسال کد فعال سازی',
  'send_verify_email' => 'ارسال کد فعال سازی',
  'completion_edit_information' => 'ویرایش و تکمیل اطلاعات',
  'edit_add_addresses' => 'افزودن و ویرایش آدرس ها',
  'national_code_invalid' => 'کد ملی صحیح نمیباشد.',
  'phone_verified' => 'شماره همراه با موفقیت ثبت شد.',
  'code_invalid' => 'کد نامعتبر است.',
  'user_not_valid' => 'کاربر معتبر نمیباشد.',
  'email_verified' => 'ایمیل با موفقیت ثبت شد.',
  'prev' => 'قبلی',
  'next' => 'بعدی',
  'login_successful/ly' => 'ورود موفقیت آمیز بود.',
  'role_sorted' => 'نقش ها مرتب شد.',
  'slug_help' => 'آدرس کوتاه شده صفحه',
  'specific_categories_list' => 'دسته بندی های خاص',
  'pages_list' => 'لیست صفحات',
  'waiting_paid' => 'در انتظار پرداخت',
  'transaction_info' => 'اطلاعات تراکنش',
  'persian' => 'فارسی',
  'on_behalf_of' => 'از طرف',
  'meeting_date' => 'تاریخ مراسم',
  'late_name' => 'نام مرحوم',
  'condolences_to' => 'تسلیت به',
  'meeting_time' => 'ساعت مراسم',
  'meeting_address' => 'آدرس مراسم',
  'descriptions' => 'توضیحات',
  'champions_payments' => 'پرداخت های کمپین ها',
  'unavailable' => 'ناموجود',
  'shop_order' => 'خرید تاج گل',
  'monthlyPayGuide' => 'شما می‌توانید از قسمت همیاری در منو سایت، تعهد پرداخت دوره‌ای جدید ایجاد کنید.',
  'you_are_login' => 'شما وارد سایت شدید',
  'pay_date' => 'تاریخ پرداخت',
  'new_charity_period' => 'ثبت کمک ماهانه جدید',
  'you_have_unpaid_period' => 'نیکوکار گرامی: شما کمک در انتظار پرداخت دارید.',
  'awardNotReceived' => 'لوح تقدیر دریافت نکرده است',
  'awardReceived' => 'لوح تقدیر دریافت شده',
  'name_op' => 'نام نیکوکار',
  'email_op' => 'ایمیل (اختیاری)',
  'phone_op' => 'تلفن همراه(اختياری،جهت دریافت تاییدیه و ‌سوابق نیکوکاری)',
  'mobile-app-manage' => 'مدیریت اپلیکیشن',
  'mobile-app' => 'اپلیکیشن موبایل',
  'next_payment' => 'نوبت پرداخت بعد',
  'map_position' => 'موقعیت مکانی',
  'user_information' => 'اطلاعات کاربر',
  'sumAll' => 'جمع کل',
  'password_reset_title' => 'بازیابی کلمه عبور',
  'password_reset_request' => 'دریافت کد بازیابی',
  'confirmation_code' => 'کد تایید',
  'charity_titel' => 'همیاری',
  'routine_type' => 'نوع کمک ماهانه/هفتگی',
  'routine_status' => 'وضعیت کمک ماهانه/هفتگی',
  'routine_is_disable' => 'شما کمک ماهانه یا هفتگی فعال ندارید',
  'routine_is_enable' => 'کمک ماهانه یا هفتگی شما فعال است',
);
