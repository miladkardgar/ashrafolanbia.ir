                ...
                <div class="card">
                    <div class="card-header">
                        <h6 class="card-title text-black">{{__('messages.Charity')}}
                            | {{__('messages.champions_payments')}}</h6>
                        <hr>
                    </div>
                            ...
                            @foreach($champion as $payment)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        {{$payment['user_id']!=0?$payment['user']['people']['name']." ".$payment['user']['people']['family']:''}}
                                    </td>
                                    <td>{{$payment['phone']}}</td>
                                    ...
                                    <td>
                                        @if($payment['status']=='paid')
                                            <span
                                                class="badge badge-success">{{__('messages.'.$payment['status'])}}</span>
                                        @elseif($payment['status']=='fail')
                                            <span
                                                class="badge badge-danger">{{__('messages.'.$payment['status'])}}</span>
                                        @else
