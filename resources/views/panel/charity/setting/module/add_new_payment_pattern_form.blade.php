...
<form method="POST" id="" class="form-ajax-submit" action="{{route('charity_payment_pattern_add')}}"
    ...
    @if($payment_pattern)
        ...
        <h4>بابت:</h4>
        <div class="row mt-2">
            @foreach(\App\charity_payment_title::all() as $title)
                <div class="col-md-3">
                    <label for="Charity_title_{{$title['id']}}" class="label-default"> {{ $title['title'] }} </label>
                    <input name="charity_title[]" id="Charity_title_{{$title['id']}}" value="{{$title['id']}}" type="checkbox" {{in_array($title['id'],$titles)? "checked":""}} class="pull-right">
                </div>
            @endforeach
        </div>
        <hr>
    ...
    <div class="row mt-2">
        <button type="submit" class="btn pull-right btn-lg btn-info">
